  ...
  <div>
      ...
      <a-table :columns="columns" :data-source="list" :loading="loading" row-key="id" :pagination="pagination" @change="onTableChange">
        <template #bodyCell="{ column, record }">
          ...
          <template v-if="column.key === 'genre'">
            <a-tag color="blue">{{ record.genre || '未分类' }}</a-tag>
          </template>
    ...
    <a-modal v-model:open="formVisible" :title="editingId ? '编辑艺人' : '新增艺人'" :confirm-loading="submitting" @ok="onSubmit" width="500px">
      <a-form :model="formData" :label-col="{ span: 5 }" :wrapper-col="{ span: 18 }">
        ...
        <a-form-item label="风格流派"><a-input v-model:value="formData.genre" placeholder="如：独立摇滚" /></a-form-item>
...
const formData = reactive({ name: '', avatarUrl: '', genre: '', description: '' })

function openForm(record?: PerformerItem) {
  if (record) { editingId.value = record.id; Object.assign(formData, record) }
  else { editingId.value = null; formData.name = ''; formData.avatarUrl = ''; formData.genre = ''; formData.description = '' }
